...
`src/api/server.py`'s existing `secrets_factory(namespace="hcr",
agent_name="hcr-c2-054")` call shape rather than a per-template value: one
Marketplace Pod deploys exactly one template, so there is no cross-template
secret-path collision to guard against, and `hcr` is filled in at
...
if __name__ == "__main__":
    run_agent_marketplace(
        Graph,  # rename together with src/graph/graph.py's class name during design (HcrLtcQaGraph)
        agent_name="hcr-c2-054",
        namespace="hcr",
